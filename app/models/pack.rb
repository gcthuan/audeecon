class Pack
  ...
  def self.initialize_packs
    ...
    data.fetch("payload").fetch("packs").collect do |index|
      ...
      current_pack = Pack.create(id: "#{pack_id}", name: "#{pack_data.fetch("name")}", artist: "#{pack_data.fetch("artist")}", description: "#{pack_data.fetch("description")}", previews: previews)
      ...
      json_stickers.fetch("payload").collect do |index|
        current_pack.stickers.create(id: "#{index.fetch("id")}", uri: "#{index.fetch("uri")}")
      end
    end
  end
end
